...
import 'package:opencentric_lms/repository/assignment_repository.dart';
...
class AssignmentController extends GetxController implements GetxService {
    ...
    if (response != null && response.statusCode == 200) {
      if (response.body['success'] == true) {
        List<Assignments> list =
            AssignmentModel.fromJson(response.body).data!.assignments!;
  ...
  Future<void> submitAssignment({required int? assignmentId}) async {
    final response = await AssignmentRepository(Get.find())
        .submitAssignment(assignmentId: assignmentId.toString(), file: file);
    if (response != null && response.statusCode == 200) {
