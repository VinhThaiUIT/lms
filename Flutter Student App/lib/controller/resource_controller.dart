...
import 'package:opencentric_lms/data/model/resource/resource_model.dart';
import 'package:opencentric_lms/repository/resource_repository.dart';
...
class ResourceController extends GetxController implements GetxService {
  ...
  Future<void> getResourceList(String courseID) async {
    ...
    update();
    final response =
        await ResourceRepository(Get.find()).getResourceList(courseID);
    if (response != null && response.statusCode == 200) {
      if (response.body['success'] == true) {
        List<Resources> list =
            ResourcesModel.fromJson(response.body).data!.resources!;
