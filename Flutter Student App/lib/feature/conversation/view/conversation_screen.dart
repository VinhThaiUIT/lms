...
import 'package:opencentric_lms/feature/conversation/widgets/chat_app_bar.dart';
import 'package:opencentric_lms/feature/conversation/widgets/input_widget.dart';
import 'package:opencentric_lms/feature/conversation/widgets/video_message_builder.dart';
...
class _ChatScreenState extends State<ConversationScreen>
  ...
  void _handleSendPressed() async {
    ...
    await ChatRepository(Get.find())
        .sendMessage(receiverId: _receiver.id, textMessage: textMessage);
    _addMessage(textMessage);
  }

  void _handleMessageTap(BuildContext _, types.Message message) async {
    if (message is types.VideoMessage) {
      Navigator.pushNamed(context, RouteHelper.videoPlayerScreen,
  ...
  void _handleImageSelection() async {
    ...
    if (result != null) {
      ...
      _addMessage(message);
      await ChatRepository(Get.find())
          .sendFile(receiverId: _receiver.id, file: File(result.path));
      _hideBottomSheet();
    }
  }
  ...
  void _handleFileSelection() async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      _addMessage(message);
      await ChatRepository(Get.find()).sendFile(
          receiverId: _receiver.id, file: File(result.files.single.path!));
      _hideBottomSheet();
      setState(() {
  ...
  Future<void> _handleEndReached() async {
    final response = await ChatRepository(Get.find())
        .getAllMessages(discussionId: chatUser.chatRoomId, page: _page);
  ...
  void startTimer() async {
    _timer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      final response = await ChatRepository(Get.find())
          .getAllMessages(discussionId: chatUser.chatRoomId, page: 1);

      if (response != null) {
        if (response.last.id.toString() != _messages.first.id &&
            response.last.type != '2') {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Chat(
          ...
          onSendPressed: (p0) {},
          user: _user,
          bubbleRtlAlignment: Get.find<LocalizationController>().isLtr
